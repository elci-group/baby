...
fn install_binary(config: &InstallConfig, from: &Path, to: &Path) -> Result<()> {
    ...
    let mut staged = stage_install_file(from, to, parent)?;

    // Retry on ETXTBSY: the target may be executing, and some kernels/filesystems
    // briefly reject the atomic rename while the executable is being loaded.
    let mut last_error = None;
    for attempt in 0..10 {
        match staged.persist(to) {
            Ok(_) => {
                last_error = None;
                break;
            }
            Err(e) if e.error.kind() == std::io::ErrorKind::ExecutableFileBusy && attempt < 9 => {
                last_error = Some(e.error);
                let delay = (10u64 * (1 << attempt)).min(100);
                std::thread::sleep(std::time::Duration::from_millis(delay));
                // persist consumed `staged` on failure, so recreate it.
                staged = stage_install_file(from, to, parent)?;
            }
            Err(e) => {
                last_error = Some(e.error);
                break;
    ...
    if let Some(error) = last_error {
        ...
        return Err(BabyError::io(context, error));
    }

    Ok(())
}

/// Create a temporary file in `parent` containing the contents of `from`,
/// ready to be atomically renamed to `to`.
fn stage_install_file(from: &Path, to: &Path, parent: &Path) -> Result<tempfile::NamedTempFile> {
    let mut staged = tempfile::Builder::new()
        .prefix(&format!(
            ".{}.",
            to.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("baby-install")
        ))
        .tempfile_in(parent)
        .map_err(|e| BabyError::io(format!("create temp file in {}", parent.display()), e))?;

    let mut source =
        fs::File::open(from).map_err(|e| BabyError::io(format!("open {}", from.display()), e))?;
...
fn processes_executing(path: &Path) -> Vec<(u32, String)> {
    ...
    for entry in entries.flatten() {
        ...
        if exe_target == canonical {
            let comm = fs::read_to_string(entry.path().join("comm")).unwrap_or_else(|_| "?".into());
...
mod tests {
    ...
    fn install_binary_overwrites_a_running_target() {
        ...
        let mut child = Command::new(&old).arg("10").spawn().unwrap();
        // Wait until the child has finished execve and is actually running the
        // target binary. Renaming over a file while the kernel is still loading
        // it can race and produce ETXTBSY on some systems.
        let pid = child.id();
        let canonical_old = fs::canonicalize(&old).unwrap();
        let start = std::time::Instant::now();
        while start.elapsed() < std::time::Duration::from_secs(2) {
            if let Ok(exe) = fs::read_link(format!("/proc/{pid}/exe")) {
                if exe == canonical_old {
                    break;
                }
            }
